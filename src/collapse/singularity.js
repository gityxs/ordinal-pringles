function updateSingularityHTML(){
    DOM(`singCostText`).innerHTML = `You have <span style="color: goldenrod">${data.incrementy.charge} 个电荷</span>`
    for (let i = 0; i < data.sing.hasEverHadFunction.length; i++) {
        ...
        if(data.sing.hasEverHadFunction[i] && singFunctions[i].hasEffect){
            hasPermanentFunction(i) ? DOM(`singFunction${i}`).innerHTML = `<span style="color: #0bce8a">奇异密度 ${ordinalDisplay('H', singFunctions[i].requiredLevel, 0, 10, 3, false)}：</span> ${singFunctions[i].hasUnlock ? `${singFunctions[i].unlockDescription} ${singFunctions[i].hasEffect ? 'and' : ''}` : ''} ${singFunctions[i].hasEffect ? `${singFunctions[i].effectDescription} ${format(singFunctions[i].effect())}` : ``}`
                : DOM(`singFunction${i}`).innerHTML = `<span style="color: #80ce0b">奇异密度 ${ordinalDisplay('H', singFunctions[i].requiredLevel, 0, 10, ordinalDisplayTrim(3), false)}：</span> ${singFunctions[i].hasUnlock ? `${singFunctions[i].unlockDescription} ${singFunctions[i].hasEffect ? 'and' : ''}` : ''} ${singFunctions[i].hasEffect ? `${singFunctions[i].effectDescription} ${format(singFunctions[i].effect())}` : ``}`
        }
    }
...
function updateSingFunctionHTML(i){
    ...
    if(data.sing.hasEverHadFunction[i]){
        DOM(`singFunction${i}`).innerHTML =
            `<span style="color: #80ce0b">奇异密度 ${ordinalDisplay('H', singFunctions[i].requiredLevel, 0, 10, ordinalDisplayTrim(3), false)}：</span> ${singFunctions[i].hasUnlock ? `${singFunctions[i].unlockDescription} ${singFunctions[i].hasEffect ? 'and' : ''}` : ''} ${singFunctions[i].hasEffect ? `${singFunctions[i].effectDescription} ${format(singFunctions[i].effect())}` : ``}`
        if((i+1 < data.sing.hasEverHadFunction.length) && !data.sing.hasEverHadFunction[i+1] && !i+1 >= data.sing.level)
            DOM(`singFunction${i+1}`).innerHTML = `<span style="color: #80ce0b">奇异密度 ${ordinalDisplay('H', singFunctions[i+1].requiredLevel, 0, 10, ordinalDisplayTrim(3), false)}：</span> ?????????????????????????`
    }
}
function checkPermanentFunction(i){
    if(hasPermanentFunction(i))
        DOM(`singFunction${i}`).innerHTML = `<span style="color: #0bce8a">奇异密度 ${ordinalDisplay('H', singFunctions[i].requiredLevel, 0, 10, ordinalDisplayTrim(3), false)}：</span> ${singFunctions[i].hasUnlock ? `${singFunctions[i].unlockDescription} ${singFunctions[i].hasEffect ? 'and' : ''}` : ''} ${singFunctions[i].hasEffect ? `${singFunctions[i].effectDescription} ${format(singFunctions[i].effect())}` : ``}`
}
...
function initSingularityFunctions(){
    for (let i = 0; i < singFunctions.length; i++) {
        ...
        el.innerHTML = hasPermanentFunction(i) ?
            `<span style="color: #0bce9a">奇异密度 ${ordinalDisplay('H', singFunctions[i].requiredLevel, 0, 10, ordinalDisplayTrim(3), false)}：</span> ${singFunctions[i].hasUnlock ? `${singFunctions[i].unlockDescription} ${singFunctions[i].hasEffect ? 'and' : ''}` : ''} ${singFunctions[i].hasEffect ? `${singFunctions[i].effectDescription} ${format(singFunctions[i].effect())}` : ``}`
            : data.sing.hasEverHadFunction[i]
            ? `<span style="color: #80ce0b">奇异密度 ${ordinalDisplay('H', singFunctions[i].requiredLevel, 0, 10, ordinalDisplayTrim(3), false)}：</span> ${singFunctions[i].hasUnlock ? `${singFunctions[i].unlockDescription} ${singFunctions[i].hasEffect ? 'and' : ''}` : ''} ${singFunctions[i].hasEffect ? `${singFunctions[i].effectDescription} ${format(singFunctions[i].effect())}` : ``}`
            : data.sing.hasEverHadFunction[i-1] || i===0 ? `<span style="color: #80ce0b">奇异密度 ${ordinalDisplay('H', singFunctions[i].requiredLevel, 0, 10, ordinalDisplayTrim(3), false)}：</span> ?????????????????????????` : `?????????????????????????`
...
let singFunctions = [
    ...
    {requiredLevel: 20, hasUnlock: true, unlockDescription: 'Keep Hierarchy Upgrades on Collapse', hasEffect: true, effectDescription: 'Negative Charge multiplies ℵ<sub>5</sub> 乘以', effect: () => Math.max(1, Math.log10(data.darkness.negativeCharge)), canBePerm: true, permReq: () => hasAOMilestone(0)},
    {requiredLevel: 35, hasUnlock: true, unlockDescription: 'Unlock an AutoBuyer for Supercharge', hasEffect: true, effectDescription: 'Total Boosters over 12246 multiply the second Overcharge Effect by<i></i>', effect: () => Math.max(1, Math.log10(Math.max(1, data.boost.total-12246))/2), canBePerm: false, permReq: () => false},
    {requiredLevel: 50, hasUnlock: true, unlockDescription: 'Double the effect of IUP4', hasEffect: true, effectDescription: 'RUP2 multiplies Dynamic Cap (at an extremely reduced rate) by<i></i>', effect: () => Math.min(Math.max(1, Decimal.log10(iup2Effect()).toNumber()), Number.MAX_VALUE), canBePerm: false, permReq: () => false},
    {requiredLevel: 65, hasUnlock: false, unlockDescription: null, hasEffect: true, effectDescription: 'Negative Charge multiplies the Total ℵ effect by<i></i>', effect: () => Math.max(1, Math.log2(data.darkness.negativeCharge)), canBePerm: false, permReq: () => false},
